...
int txtfscanf(char *txt)
{ /* aux function to check for text "txt" on ifile */
  char inp[256];
  int ignore;

  ignore=fscanf(ifile, "%s ", inp);
  if (strcmp(inp,txt) != 0) /* does not match */
    return(1);
  else
    return(0);
}
...
{
 if (txtfscanf(txt) != 0)
    fserror ("scanFloat","reading literal ",txt);
...
{
   if (txtfscanf(txt) != 0)
    fserror ("scanDouble","reading literal ",txt);
...
{
   if (txtfscanf(txt) != 0)
    fserror ("fscanDouble","reading literal ",txt);
  if (fscanf(ifile," %lf ",f) != 1) {
...
void scanInt(char *txt, int *n)
{
  int 	s;
   ...
   if (txtfscanf(txt) != 0)
    fserror ("scanInt","reading literal ",txt);
...
void fscanInt(FILE *ifile, char *txt, int *n)
{
   ...
   if (txtfscanf(txt) != 0)
    fserror ("fscanInt","reading literal ",txt);
  if ((s = fscanf(ifile," %d ",n)) != 1) {
...
void scanLong(char *txt, long *n)
{
   ...
   if (txtfscanf(txt) != 0)
    fserror ("scanLong","reading literal ",txt);
...
void scanArray(char *txt, float *x, int n)
{
   ...
   if (txtfscanf(txt) != 0)
    fserror ("scanArray","reading literal ",txt);
  for(i=0;i<n;i++){
...
void scanDoubleArray(char *txt, double *x, int n)
{
  ...
  if(txt != 0){
    if (txtfscanf(txt) != 0)
      fserror ("scanDoubleArray","reading literal ",txt);
  }
  for(i=0;i<n;i++){
...
void scanDoubleMatrix(char *txt,double **x,int r,int c)
{
  ...
  if (txt!=0){
    if (txtfscanf(txt) != 0)
      fserror ("scanDoubleMatrix","reading literal ",txt);
  }
  for(i=0;i<r;i++)
    for(j=0;j<c;j++){
...
void scanIntArray(char *txt, int *x, int n)
{
  int	i;
   ...
   if (txtfscanf(txt) != 0)
    fserror ("scanIntArray","reading literal ",txt);
  for(i=0;i<n;i++){
    if (fscanf(ifile," %d ",&x[i]) != 1) {
      fserror ("scanIntArray","reading int array",txt);
    }
  }
}
...
void fscanIntArray(FILE *ifile, int *x, int n)
{
  ...
  for(i=0;i<n;i++){
    if (fscanf(ifile," %d ",&x[i]) != 1) {
      fserror("fscanIntArray","reading int array","");
    }
  }
}
